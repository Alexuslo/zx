...
{                                                               \
        (x) = ((CContext *) Context)->Memory[(address) & 0xffff];\
}
...
{                                                                       \
	unsigned char	*Memory;					\
									\
	Memory = ((CContext *) Context)->Memory;				\
...
{                                                                       \
        if (address > 16383) ((CContext *) Context)->Memory[(address) & 0xffff] = (x); \
}
...
{                                                                       \
														\
		x =	ReadPort(Port);			\
      /* SystemCall((CContext *)Context);*/							\
}

#define Z80_OUTPUT_BYTE(Port, x)                                        \
{                                                                       \
		WritePort(Port, x);																\
        ((CContext *) Context)->IsDone = !0; 							\
        nCycles = 0;													\
}
