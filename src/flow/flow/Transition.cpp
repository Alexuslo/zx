...
namespace oxygine
{
    ...
    namespace flow
    {
        Color _fadeColor = Color(0, 0, 0, 210);
		...
		void TransitionMove::assign(spScene scene)
		{
			TransitionMove::assign((scene).get());
		}
		...
		TransitionMove::TransitionMove():_moveIn(true)
        {
            ...
            _srcOut = _srcIn = Vector2(0.0f, -getStage()->getHeight());
        ...
        {
            Vector2 src = back ? _srcOut : _srcIn; 
		...
		void TransitionFade::assign(spScene scene)
		{
			TransitionFade::assign((scene).get());
		}
		...
		void TransitionFadeFromPoint::assign(Scene *scene)
		{
			spTransition t = new TransitionFadeFromPoint;
			scene->setTransitionIn(t);
			scene->setTransitionOut(t);
		}

		void TransitionFadeFromPoint::assign(spScene scene)
		{
			TransitionFadeFromPoint::assign((scene).get());
		}

		TransitionFadeFromPoint::TransitionFadeFromPoint()
		{
			_fade = new ColorRectSprite;
			_fade->setPosition(-10000, -10000);
			_fade->setSize(Vector2(30000, 30000));
		...
		void TransitionFadeFromPoint::_run(spScene current, spScene next, bool back)
		{
			spScene target = back ? current : next;
			spActor holder = target->getHolder();
			...
			{
				target->getHolder()->setScale(0.0f, 0.0f);
				_fade->setAlpha(0);
				_fade->addTween(Actor::TweenAlpha(255), _tweenOpt._duration);
				holder->insertSiblingBefore(_fade);
			}
			...
			timeMS duration = _tweenOpt._duration / 2;
			
			spTween tween = target->getHolder()->addTween(TweenAlphaFade(!back, PostProcessOptions().fullscreen().clear(cl)), _tweenOpt._duration, 1, false, 0);
			spTween tweenS = target->getHolder()->addTween(Actor::TweenScale(static_cast<float>(!back)), duration, 1, false, back ? duration : 0);
			spTween tweenD = target->getHolder()->addTween(CTweenSpring(1.0f, false, 0.25f, 0.25, back ? 1 : 2), duration, 1, false, back ? 0 : duration);
			waitTween(tween);
		}
		...
		void TransitionShutters::assign(spScene scene)
		{
			TransitionShutters::assign((scene).get());
		}
		...
		void TransitionQuads::assign(spScene scene)
		{
			TransitionQuads::assign((scene).get());
		}
